...
void
LLMInference::loadModelFromBuffer(void* buffer, size_t size, float minP, float temperature, bool storeChats,
                                 long contextSize, const char* chatTemplate, int nThreads, bool useMlock) {
    LOGi("loadModelFromBuffer: starting with size %zu", size);

    // Create a virtual file from the memory buffer
    FILE* file = fmemopen(buffer, size, "rb");
    if (file == nullptr) {
        LOGe("loadModelFromBuffer: fmemopen failed: %s", strerror(errno));
        throw std::runtime_error("loadModelFromBuffer: fmemopen failed");
    }
    LOGi("loadModelFromBuffer: fmemopen success");

    // Get the file descriptor for the virtual file
    int fd = fileno(file);
    char path[256];
    sprintf(path, "/proc/self/fd/%d", fd);
    LOGi("loadModelFromBuffer: virtual path created: %s", path);

    // load dynamic backends
    ggml_backend_load_all();
    LOGi("loadModelFromBuffer: ggml_backend_load_all success");

    llama_model_params model_params = llama_model_default_params();
    model_params.use_mmap           = true;
    model_params.use_mlock          = useMlock;
    LOGi("loadModelFromBuffer: calling llama_model_load_from_file with path %s", path);
    
    // llama.cpp will open /proc/self/fd/N which refers to our memory buffer
    _model                          = llama_model_load_from_file(path, model_params);
    
    if (!_model) {
        LOGe("loadModelFromBuffer: llama_model_load_from_file returned NULL for path %s", path);
        fclose(file);
        throw std::runtime_error("loadModelFromBuffer() failed");
    }
    LOGi("loadModelFromBuffer: llama_model_load_from_file success");
